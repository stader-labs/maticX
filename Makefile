...
POLYGON_STAGING_DEPLOYER := 0x6FCBE6C2d6f6c890fD0cC5b0288E2D474D90BB1F
...
deploy-validatorregistry-ethereum-staging:
	$(BIN_HARDHAT) deploy:validator-registry --network $(NETWORK_ETHEREUM) --stake-manager $(ETHEREUM_STAGING_STAKE_MANAGER) --matic $(ETHEREUM_STAGING_MATIC) --matic-x $(ETHEREUM_STAGING_MATIC_X) --manager $(ETHEREUM_STAGING_MANAGER)
...
deploy-maticx-ethereum-staging:
	$(BIN_HARDHAT) deploy:matic-x --network $(NETWORK_ETHEREUM) --validator-registry $(ETHEREUM_STAGING_VALIDATOR_REGISTRY) --stake-manager $(ETHEREUM_STAGING_STAKE_MANAGER) --matic $(ETHEREUM_STAGING_MATIC) --manager $(ETHEREUM_STAGING_MANAGER) --treasury $(ETHEREUM_STAGING_TREASURY)
...
deploy-timelockcontroller-ethereum-staging:
	$(BIN_HARDHAT) deploy:timelock-controller --network $(NETWORK_ETHEREUM) --min-delay $(TIMELOCK_CONTROLLER_SHORT_DELAY) --manager $(ETHEREUM_STAGING_MANAGER)
...
verify-timelockcontroller-ethereum-staging:
	$(BIN_HARDHAT) verify-contract --network $(NETWORK_ETHEREUM) --contract $(ETHEREUM_STAGING_TIMELOCK_CONTROLLER) "$(TIMELOCK_CONTROLLER_SHORT_DELAY)" "[$(ETHEREUM_STAGING_MANAGER)]" "[$(ETHEREUM_STAGING_MANAGER)]" "$(ETHEREUM_STAGING_MANAGER)"
...
initializev2-validatorregistry-ethereum-staging:
	$(BIN_HARDHAT) initialize-v2:validator-registry --network $(NETWORK_ETHEREUM) --contract $(ETHEREUM_STAGING_VALIDATOR_REGISTRY) --pol $(ETHEREUM_STAGING_POL)
